...
function LogInSignUp({ logIn, isLoggedIn }) {
  ...
  return (
    <div>
      ...
      <button className="btn btn-primary" disabled={isLoginForm} onClick={() => setIsLoginForm(true)}>Login</button>
      <button className="btn btn-primary" style={{margin:"1rem"}} disabled={!isLoginForm} onClick={() => setIsLoginForm(false)}>SignUp</button>
